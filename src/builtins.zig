...
pub fn registerAll(vm: *Vm) !void {
    try vm.env.global.registerFunction(vm, &DEFINE_FUNCTION);
    try vm.env.global.registerFunction(vm, &PLUS_FUNCTION);
}
...
fn defineImpl(vm: *Vm) Val.FunctionError!Val {
    ...
    try vm.env.global.registerValue(vm, symbol, value);
    return Val.init();
}
